...
# fit svs to into annotation using taonomy table genrated by qiime2. 
sv_2_tax_v3 = function(svs, latest_db_num = 29, gg2_flag = T)
{
  taxonomy_file = sprintf('/pita/users/tzipi/projects/16S/all_merges/DB1-%s_merge/res/16S_DB1-%s_merged/taxonomy/taxonomy_gg1_source_risk.tsv', latest_db_num, latest_db_num)
  if ( gg2_flag )
    taxonomy_file = gsub('gg1','gg2', taxonomy_file)
  txn = read.table(file = taxonomy_file, header = T, sep = '\t')
  temp = data.frame(Feature_ID = as.character(svs))
  res = merge(x = temp, y = txn, by = 'Feature_ID', all.x = TRUE, sort = F)
  res$Taxon = gsub(pattern = ';_',replacement = '; ', res$Taxon)
        ...
        fdr = 0.25, taxonomy_flag = T)
{
  ...
  # taxa_m$log10_RA = taxa_m$value  
  
  ## order by coefficient and the variable of interest
  taxa_m = merge(taxa_m, df[,c('ASV','coef')], by='ASV')
  temp = unique(taxa_m[, c('taxa','coef')])
  # in case of several groups, more than one coefficient per taxa 
  if ( max( table(temp$taxa)) >1 )
  {
    # using the strongest coefficiaent for the order
    for ( i in 1:dim(temp)[1] )
      temp$coef[i] = max( abs(temp$coef[temp$taxa == temp$taxa[i]] ) )
    temp = unique(temp) 
  }
                                  ...
                                  out_dir_name = 'my_figures/', add_name = '', taxa_sig_var = '', fdr = 0.25, taxonomy_flag = T)
{
  ...
  return(list(p_heatmap, taxa_m))
}


maaslin2_healtmap_plot2 = function(taxa, head_path, maas_name, x_var, group_var = '', 
                                  taxonomy_file = '/pita/users/tzipi/projects/16S/all_merges/DB1-20_merge/res/16S_DB1-20_merged/taxonomy/metadata_v2_source.tsv', 
                                  out_dir_name = 'my_figures/', add_name = '', taxa_sig_var = '', fdr = 0.25, taxonomy_flag = T, q_25_1_flag =T)
{
  if (taxa_sig_var == '')
    taxa_sig_var = x_var
  taxa_m = maaslin2_set_data_for_heatmap_plot(taxa, head_path, maas_name, x_var, taxa_sig_var, group_var = group_var, taxonomy_file = taxonomy_file, fdr,taxonomy_flag = taxonomy_flag)
  taxa_m2 = maaslin2_set_data_for_heatmap_plot(taxa, head_path, maas_name, x_var, taxa_sig_var, group_var = group_var, taxonomy_file = taxonomy_file, 0.1,taxonomy_flag = taxonomy_flag)
  
  taxa_m$qval_cutoff = ifelse(taxa_m$ASV %in% taxa_m2$ASV,'Q<=0.1','0.1<Q<=0.25')
  ...
  if (group_var != '')
    p_heatmap = p_heatmap + facet_grid(as.formula(sprintf('.~%s',group_var)), scales = 'free_x', space = 'free') 
  library(ggside)
  if ( q_25_1_flag )
    p_heatmap = p_heatmap + geom_ysidepoint(aes(colour = qval_cutoff)) + ggside(collapse = "y") + 
      theme(ggside.panel.scale = 0.05, panel.spacing = unit(1, "lines")) + scale_colour_manual(values = c('gray50','black'), name='')
  out_path = sprintf('%s/%s/res/%s',head_path,maas_name, out_dir_name)
  dir.create(out_path)
  out_fig = sprintf('%s/%s_sigTaxa_%s_%s_heatmap%s_fdr%s.tiff', out_path, taxa_sig_var, x_var, group_var, add_name, fdr)
  ggsave(out_fig,plot = p_heatmap, device = 'tiff', width = 12,height = 2+length(unique(taxa_m$ASV))/8, compression  = 'lzw')
