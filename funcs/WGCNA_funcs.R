...
WGCNA_calculate_module_eigengenes_and_corrs = function(ftr_df,moduleColors, metadata_df, cor_type = 'WGCNA_default', name)
{
  ...
  { ## calculate spearman correlation instead
    ...
    for (i in 1:dim(MEs)[2])
    {
      ...
      {
        res = cor.test(x=MEs[,i], y= metadata_df[,j],method = 'spearman', na.rm=T)
...
WGCNA_calculate_module_PC1_and_corrs = function(ftr_df,moduleColors, metadata_df, cor_type = 'WGCNA_default', name)
{
  ## get original MEs just for the format
  MEs = WGCNA::orderMEs( WGCNA::moduleEigengenes(ftr_df, moduleColors)$eigengenes )
  ## calculate zscore means instead
  moduleColors2 = sprintf('ME%s', moduleColors)
  ftr_df2 = as.data.frame(scale(ftr_df), center = T, scale = T)
  # ftr_df2 = ftr_df
  # for (i in 1:dim(MEs)[1])
  # {
    for (j in 1:dim(MEs)[2])
    {
      temp = prcomp(ftr_df2[, moduleColors2 == names(MEs)[j] ], 
                               center = F, scale = F)
      MEs[,j] = temp$x[,'PC1']
    }
  # }
...
WGCNA_calculate_module_zscore_mean_and_corrs = function(ftr_df,moduleColors, metadata_df, cor_type = 'WGCNA_default', name)
{
  ## get original MEs just for the format
  MEs = WGCNA::orderMEs( WGCNA::moduleEigengenes(ftr_df, moduleColors)$eigengenes )
  ## calculate PC1 instead
  moduleColors2 = sprintf('ME%s', moduleColors)
  ftr_df2 = as.data.frame(scale(ftr_df), center = T, scale = T)
  for (i in 1:dim(MEs)[1])
  {
    for (j in 1:dim(MEs)[2])
    {
      MEs[i,j] = mean( as.numeric(ftr_df2[row.names(MEs)[i], 
                                          moduleColors2 == names(MEs)[j] ] ) )
    }
  }
  if ( cor_type == 'WGCNA_default')
  { ## use the default pearson + Student p value
...
WGCNA_ME_met_heatmap_original_prm_related = function(moduleTraitCor, moduleTraitPvalue, name, prm, cutoff, path, rm_grey_flag = T)
{
  good_cols = row.names(moduleTraitPvalue)[ moduleTraitPvalue[,prm] <= cutoff ]
  if (rm_grey_flag)
    good_cols = good_cols[good_cols!='MEgrey']
...
WGCNA_ME_met_heatmap_full_res = function(moduleTraitCor, moduleTraitPvalue, name, metadata_prms = c(), min_p = 1, wanted_modules_order = c(), fdr_flag_non_metadata = F, cluster_cols_order_flag = T, show_p_value_fdr_flag = F)
{
  ...
  if (fdr_flag_non_metadata) ## add fdr on non-metadata varibale (not in metadata_prms)
  {
    ...
    mdc$q_value[pos] = p.adjust(mdc$p_value[pos], method = 'BH', n = length(mdc$q_value[pos]))
    ...
    {
      if ( ! all(is.na(mdc$q_value[ mdc$variable == v ] ) ) )
      {
...
{
  res_p = WGCNA_ME_met_heatmap_full_res(moduleTraitCor, moduleTraitPvalue, name, 
                                        metadata_prms, min_p = min_p, wanted_modules_order = wanted_modules_order, 
                                        fdr_flag_non_metadata = F, show_p_value_fdr_flag = show_p_value_fdr_flag )
  res_q = WGCNA_ME_met_heatmap_full_res(moduleTraitCor, moduleTraitPvalue, name, 
                                        metadata_prms, min_p = min_q, wanted_modules_order = wanted_modules_order, 
...
WGCNA_ME_met_heatmap_q = function(moduleTraitCor, moduleTraitPvalue, name, metadata_prms = c(), 
                                   wanted_modules_order = c(), # fdr_flag_non_metadata = F, 
                                   cluster_cols_order_flag = T, min_q = 1)
{
  res_q = WGCNA_ME_met_heatmap_full_res(moduleTraitCor, moduleTraitPvalue, name, 
                                        metadata_prms, min_p = min_q, wanted_modules_order = wanted_modules_order, 
...
WGCNA_ME_met_heatmap_prm_related = function(moduleTraitCor, moduleTraitPvalue, name, prm, cutoff, metadata_prms = c(), rm_grey_flag = T,wanted_modules_order = c())
{
  good_cols = row.names(moduleTraitPvalue)[ moduleTraitPvalue[,prm] <= cutoff ]
  if (rm_grey_flag)
    good_cols = good_cols[good_cols!='MEgrey']
  if ( length(good_cols >1))
    WGCNA_ME_met_heatmap(moduleTraitCor[good_cols,], moduleTraitPvalue[good_cols,], sprintf('%s_%sF%s',name,prm, cutoff), metadata_prms, wanted_modules_order =  wanted_modules_order)
}


WGCNA_ME_met_heatmap_prm_related_pflt = function(moduleTraitCor, moduleTraitPvalue, name, prm, cutoff = 0.05, metadata_prms = c(), p_cutoff = 0.05, rm_grey_flag = T, wanted_modules_order = c())
{
  good_cols = row.names(moduleTraitPvalue)[ moduleTraitPvalue[,prm] <= cutoff ]
  if (rm_grey_flag)
    good_cols = good_cols[good_cols!='MEgrey']
  ...
  if ( length(good_cols >1) )
    WGCNA_ME_met_heatmap(moduleTraitCor[good_cols,good_rows], moduleTraitPvalue[good_cols,good_rows], 
                         sprintf('%s_%sF%s_pF%s',name,prm, cutoff, p_cutoff), metadata_prms, wanted_modules_order = wanted_modules_order)
}
