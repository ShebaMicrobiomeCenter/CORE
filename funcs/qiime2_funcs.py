...
# classifies SVs to taxonomy using the pre-trained classifier. makes taxonomy tables at all levels 
# and taxonomic interactive barplots (taxa-bar-plots.qzv)	
def taxa_analysis_gg2_classifier(out_path, map_file, verbose = False, classifier = '/pita/users/tzipi/DB/greengenes2/greengenes2_classifier_2022.10.backbone.v4.nb.qza', decompres_flag = True, threads = 10):
	# classifies feathures (SVs) to taxa using the pre-trained classifier
	os.system('qiime feature-classifier classify-sklearn \
	--i-classifier ' + classifier + ' \
	--i-reads ' + out_path + '/rep-seqs.qza \
	--o-classification ' + out_path + '/taxonomy_gg2_classifier.qza \
	--p-n-jobs ' + str(threads) )
	# creating a visualization of the resulting table (.qzv file)
	os.system('qiime metadata tabulate \
	--m-input-file ' + out_path + '/taxonomy_gg2_classifier.qza \
	--o-visualization ' + out_path + '/taxonomy_gg2_classifier.qzv')
	# making an interactive taxonomic barplot
	os.system('qiime taxa barplot \
	--i-table ' + out_path + '/table.qza \
	--i-taxonomy ' + out_path + '/taxonomy_gg2_classifier.qza \
	--m-metadata-file ' + map_file + ' \
	--o-visualization ' + out_path + '/taxa-bar-plots_gg2_classifier.qzv')
	if decompres_flag:
		decompress_qzv(out_path)
	if verbose:
		print('taxa analysis complete gg2 classifier')
	return

# classifies SVs to taxonomy using the pre-trained classifier. makes taxonomy tables at all levels 
# and taxonomic interactive barplots (taxa-bar-plots.qzv)	
def taxa_analysis_gg2_match(out_path, map_file, verbose = False, tree = '/pita/users/tzipi/DB/greengenes2/2022.10.taxonomy.asv.nwk.qza', decompres_flag = True, threads = 10):
	# classifies feathures (SVs) to taxa using the pre-trained classifier
	os.system('qiime greengenes2 filter-features \
	--i-feature-table ' + out_path + '/table.qza \
	--i-reference ' + tree + ' \
	--o-filtered-feature-table ' + out_path + '/table_gg2_filtered.qza')
	os.system('qiime greengenes2 taxonomy-from-table \
  --i-reference-taxonomy ' + tree + ' \
  --i-table ' + out_path + '/table_gg2_filtered.qza \
  --o-classification ' + out_path + '/taxonomy_gg2_match.qza')
	# creating a visualization of the resulting table (.qzv file)
	os.system('qiime metadata tabulate \
	--m-input-file ' + out_path + '/taxonomy_gg2_match.qza \
	--o-visualization ' + out_path + '/taxonomy_gg2_match.qzv')
	# making an interactive taxonomic barplot
	os.system('qiime taxa barplot \
	--i-table ' + out_path + '/table_gg2_filtered.qza \
	--i-taxonomy ' + out_path + '/taxonomy_gg2_match.qza \
	--m-metadata-file ' + map_file + ' \
	--o-visualization ' + out_path + '/taxa-bar-plots_gg2_match.qzv')
	if decompres_flag:
		decompress_qzv(out_path)
	if verbose:
		print('taxa analysis complete')
