...
# v3 cahange direction detection from suffix (required changing the files name in many cases) to any string
def run_kallisto_paired_kal_42_5_gencode_v24_v3(in_path, out_path, add_name = '', r1_string = '_R1_', r2_string = '_R2_', kallisto_index = '/pita/users/tzipi/DB/kallisto_index/hg_gencode_v24', remove_name_start_flag = True):
    os.system('mkdir ' + out_path)

    # getting fastq files in in_folfer
    # files_r1 = glob.glob(in_path + '*_r1.fastq')
    files_r1 = glob.glob(in_path + '*' + r1_string + '*')
    # files_r2 = glob.glob(in_path + '*_r2.fastq')
    files_r2 = glob.glob(in_path + '*' + r2_string + '*')
    files_r1.sort()
    files_r2.sort()
    ...
    for i in range(0,len(files_r1)):
        ...
        np_file = files_r1[i].split('/')[-1]
        # file_name1 = np_file[0:-len(r1_suffix)]
        file_name1 = re.sub(r1_string, "", np_file)
        file_name1 = re.sub('.fastq', "", file_name1)
        #checking it is the same name as in r2
        np_file = files_r2[i].split('/')[-1]
        # file_name2 = np_file[0:-len(r2_suffix)]
        file_name2 = re.sub(r2_string, "", np_file)
        file_name2 = re.sub('.fastq', "", file_name2)
        if file_name1!=file_name2:
            print(file_name1 + ' != ' + file_name2)
        else:
            file_name1 = add_name + file_name1
