...
plot_variable_and_stats_v2 = function(taxa, check_val, y_val, sig_ast_flag = T, print_pvals = F, y_var = 'PD_whole_tree', y_lb = 'Alpha Diversity',boxplot_p_text_flag = T, boxplot_test_type = 'wilcox', jitter_flag = T, show_pval_as_asterix = T )
{
  if ( sum(!is.na(taxa[[check_val]])) == 0 )
    return( list( ggplot(),1 ) )
  # scatter plot
  taxa = taxa[!is.na(taxa[[check_val]]),]
  ...
  if ( is.logical( taxa[[ check_val ]]  ))
    taxa[[ check_val ]] = as.character( taxa[[ check_val ]] )
  ...
  return( list(p, min_p_val) )
}

# plots scatter plot if check_val is numeric and box plot if categoric. y should be numeric.
plot_variable_and_stats_v3 = function(taxa, check_val, y_val, sig_ast_flag = T, print_pvals = F, y_var = 'PD_whole_tree', y_lb = 'Alpha Diversity',boxplot_p_text_flag = T, boxplot_test_type = 'wilcox', jitter_flag = T, show_pval_as_asterix = T )
{
  if ( sum(!is.na(taxa[[check_val]])) == 0 )
    return( list( ggplot(),1 ) )
  # scatter plot
...
# for 2 variables. calculate correlatiom if both are numeric and wilcox
# if one is categorical. for 2 categorical return NA
calc_2_vars_corr_or_wilcox = function(x,y)
{
  if ( sum(!is.na(x)) == 0 | sum(!is.na(y)) == 0 | 
       sum( !is.na(x) & !is.na(y) ) < 10 )
    return( list( NA,NA ) )

  if ( is.numeric( x ) & is.numeric( y ) )
  {
    res = cor.test(x=x, y=y, method = 'spearman')
    return( list( res$estimate, res$p.value ))
  }
  if ( is.numeric(y) & !is.numeric(x) ) # make sure x is the numeric if one is not 
    temp = x; x=y; y=temp
  if ( is.logical( y ))
    y =as.character(y)
  if ( is.character( y ) )
  {
    res  = pairwise.wilcox.test(x,y, p.adjust.method = "BH")
    return(list(NA, min(res$p.value, na.rm = T)))
  }
  return( list(NA,NA) )
}
...
add_significant_asterix_to_plot_BH_v2 = function(p, var, val, p_val_cutoff = 0.05, line_width = 1, print_pvals = F, log_scale_flag=F, test_type = 'perm',label_size = 8, scale_var = 6, asterix_scale_var = 1.3, manual_label = '', show_pval_as_asterix = T, angle = 0)
{
  all_vars = levels(var)
      ...
      for ( j in 1:(i-1) )
      {
        ...
        if ( length(pos) != 0 ) # if there is enough data to check
        {
          ...
          if ( BH[pos] < p_val_cutoff ) # if p.value is significant enough, add asterix
          {
            ...
            # adding the asterix to plot
            p = p + geom_path(data = df2, aes(x = x_pos, y = y_pos),size = line_width) + 
              annotate("text", x = mean( c(start_pos, end_pos) ), y = astx_pos+scale*asterix_scale_var, label = lbl, size = label_size, angle = angle) 
...
## help function for ggaddsig
## calualtes p and q values and creats a summary dataframe to be used later
# var = the variable to divide by. needs to be factor
# val = the values
# test_type = wilcox/t.test/perm
# fdr_type = BH/none
ggaddsig_calc_pq = function(var, val, test_type = 'wilcox', fdr_type = 'BH')
{
  var = as.factor(var)
  all_vars = levels(var)
  
  p_df = data.frame(t(combn(all_vars, 2)))
  names(p_df) = c('var1','var2')
  p_df$p = NA; p_df$p1 = NA; p_df$p2 = NA; 
  
  # going over all combinations and calculating p values
  for ( i in 1:dim(p_df)[1] )
  {
    p_df$p1[i] = which(all_vars == p_df$var1[i])
    p_df$p2[i] = which(all_vars == p_df$var2[i])
    
    pos1 = var == p_df$var1[i] & !is.na(var)
    pos2 = var == p_df$var2[i] & !is.na(var)
    ...
    p_df$p[i] = res$p.value 
  }
  p_df = p_df[!is.na(p_df$p),]
  
  # calculating FDR 
  if (fdr_type == 'BH')
  {
    p_df$q = p.adjust(p_df$p,method="BH") 
  } else if ( fdr_type == 'none')
  {
    p_df$q = p_df$p
  } else
  {
    p_df$q = p_df$p
    print(sprintf('no valid FDR type (%s), p values used instead. FDR should be BH|none',fdr_type))
  }
...
## help function for ggaddsig
## organize the manual pq table to fit data to plot
# var = the variable to divide by. needs to be factor
ggaddsig_org_manual_pq = function(var, manual_pval)
{
  
  ## organize manual p values datafrmae
  p_df = manual_pval
  ## if no q, use p
  if( !'q' %in% names(p_df) & 'p' %in% names(p_df) )
    p_df$q = p_df$p
  if ( ! all(names(p_df) %in% c('var1','var2','p','q')) )
  {
    print( print('p_df col names should be: var1, var2, p, q') )
    return( list(p, p_df) )
  } 
  if ( ! all( c(p_df$var1, p_df$var2) %in% unique(var) ) )
  {
    print( print('p_df var1 and var2 should match var') )
    return( list(p, p_df) )
  }
  ## match var1 and var2 to "var" factor levels (=figure order)
  p_df$p1 = NA
  p_df$p2 = NA
  for ( i in 1:dim(p_df)[1] )
  {
...
## help function for ggaddsig
## adding p/q significantce marking on the actual plot
## using help dataframe calculated in ggaddsig_calc_pq or manually created
# p_val_cutoff = cuoff of p.value to show differnce form. default is 0.05
# line_width = width of the line. astetic thing
# val = the values used in teh plot, needed to calculate proportions
ggaddsig_plot_pq = function(p, p_df, val, 
                            p_val_cutoff = 0.05, line_width = 1, 
                            log_scale_flag = F, print_pvals = F, 
                            label_size = 8, scale_var = 6, 
                            asterix_scale_var = 1.3, next_step_scale = 2.5, 
                            show_pval_as_asterix = T, angle = 0)
{
  ## setting scale variables, will be used to determaine the proprotions of 
  ## the added significance marking
  # if using actial values and not an asterix, and not manuallly adjusting 
  # the relavant variables, choose more approproate text size + scale vars
  if (label_size == 8 & show_pval_as_asterix == F)
  {
    label_size = 3.5; scale_var = 4; asterix_scale_var = 2
  }
  if (angle == 90)
    asterix_scale_var = asterix_scale_var*2
  scale = sd(val, na.rm = T)/scale_var
  max_pos = max(val, na.rm = T) + scale

  ## adding p/q values to the figure
  for ( i in 1:dim(p_df)[1] )
  {
    if (print_pvals)
    {
      print(sprintf('%s vs %s p.val = %f , BH = %f', 
                    p_df$var1[i], p_df$var2[i], p_df$p[i], p_df$q[i] ))
    }
    if ( p_df$q[i] < p_val_cutoff ) # if p/q is significant enough, add asterix
    {
      # paramters
      astx_pos = max_pos
      start_pos = p_df$p1[i]
      ...
      lbl = find_pVal_asterix(p_df$q[i])
      if (!show_pval_as_asterix)
      {
        lbl = ifelse( p_df$q[i] < 0.009, sprintf('%.2e',p_df$q[i]), sprintf('%.4s',p_df$q[i]) )
      }
      
      # building neccecary data frame
      x_pos = c(start_pos, start_pos,end_pos,end_pos)
      y_pos = c(astx_pos, astx_pos+scale, astx_pos+scale, astx_pos)
      df2 <- data.frame(x_pos = x_pos, y_pos = y_pos )
      ...
      {
        p = p + geom_path(data = df2, aes(x = x_pos, y = y_pos),size = line_width) + 
          annotate("text", x = mean( c(start_pos, end_pos) ), 
                   y = astx_pos+scale*asterix_scale_var, 
                   label = lbl, size = label_size, angle = angle) 
      } else
        p = p + geom_path(data = df2, aes(y = x_pos, x = y_pos),size = line_width) + 
        annotate("text", y = mean( c(start_pos, end_pos) ), 
                 x = astx_pos+scale*asterix_scale_var, 
                 label = lbl, size = label_size, angle = angle, ) 
      
      # updating maximal position on plot for next time
      max_pos = max_pos + scale*next_step_scale
      if (log_scale_flag)
        max_pos = max_pos + scale*max_pos
    }
  }
...
# based on add_significant_asterix_to_plot_BH_v2
# adds significant asterix by p. value to plot, 
# can use wilcox/t.tets/permutaion test and Benjamini–Hochberg FDR
# p = the plot to add on. has to be built as ggplot(data = ?), with aes in geom_boxplot etc.
# var = the variable to divide by. needs to be factor
# val = the values
# p_val_cutoff = cuoff of p.value to show differnce form. default is 0.05
# line_width = width of the line. astetic thing
# permutaion test = added new test type - self made permutation (ranked mean difference, 1000+ permutaion, x>= my value ,x+1/x+1) test, as Amnon recommended.
ggaddsig = function(p, var, val, angle = 0, show_pval_as_asterix = T, 
                    manual_pval = data.frame(),
                    p_val_cutoff = 0.05, line_width = 1, 
                    log_scale_flag=F, print_pvals = F, 
                    test_type = 'wilcox', fdr_type = 'BH', 
                    label_size = 8, scale_var = 6, 
                    asterix_scale_var = 1.3, next_step_scale = 2.5)
{

  ## checking if manual label or calculate myself
  if (dim(manual_pval)[1] == 0)
  {
    ## calculating relavant p and q values
    p_df = ggaddsig_calc_pq(var, val, test_type, fdr_type)
    ## add calculated p/q values to figure
    p = ggaddsig_plot_pq(p, p_df, val, 
                         p_val_cutoff, line_width, 
                         log_scale_flag, print_pvals, 
                         label_size, scale_var, 
                         asterix_scale_var, next_step_scale, 
                         show_pval_as_asterix, angle)
  } else
  {
    p_df = ggaddsig_org_manual_pq(var, manual_pval)
    ## add manual p/q values to figure
  ...
  p_df = p_df[,!names(p_df) %in% c('p1','p2')]
  return( list(p, p_df) )
}
...
set_dist_map_by_vars = function( dist, map, vars)
{
  ...
  {
    wanted_samples_id = row.names(map)[ !is.na(map[[vars]]) ]
  }
  # set fit for how R reads names
  row.names(dist) = make.names(row.names(dist))
  dist_wanted_samples_id = make.names(wanted_samples_id)
...
check_var_NA_v2 = function(vars_2_check, map)
{
  var_num = vector(mode = 'character',length = length(vars_2_check))
  NA_num = vector(mode = 'numeric',length = length(vars_2_check))
  samp_num = vector(mode = 'numeric',length = length(vars_2_check))
  ...
  df = data.frame(var = vars_2_check, var_num = var_num, NA_num = NA_num, samp_num = samp_num)
  return(df)
}
...
## calculation rank for health index calculations
RA_biom_to_rank = function(SV_RA)
{
  
  SV_rank=SV_RA
  for ( i in 1:dim(SV_RA)[1] )
  {
    SV_rank[i,] = rank(SV_RA[i,])
  }
  return(SV_rank)
}

## calculating haya's microbial health index
calculate_health_index = function(SV, up_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-up_feature.txt', down_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-down_feature.txt', last_metadata = 'Description')
{
  up_df = read.table(up_file,sep="\t", header=TRUE, na.strings = na_str)
  down_df = read.table(down_file,sep="\t", header=TRUE, na.strings = na_str)
  final_SV = SV[, ( which(names(SV) == last_metadata) +1 ):dim(SV)[2] ]
  SV_up = final_SV[,names(final_SV) %in% c(up_df$Feature_ID)]
  SV_down = final_SV[,names(final_SV) %in% c(down_df$Feature_ID)]
  hi = vector( mode = 'numeric',length = dim(final_SV)[1] )
  for ( i in 1:dim(final_SV)[1])
  {
    hi[i] = log2( ( sum(SV_down[i,])+.0001 ) / (sum(SV_up[i,])+.0001 )  )
  }
  return(hi)
}

## calculating haya's microbial health index, the binary (and more common) version
calculate_binary_health_index = function(SV, up_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-up_feature.txt', down_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-down_feature.txt')
{
  up_df = read.table(up_file,sep="\t", header=TRUE, na.strings = na_str)
  down_df = read.table(down_file,sep="\t", header=TRUE, na.strings = na_str)
  SV_up = SV[,names(SV) %in% c(up_df$Feature_ID)]
  SV_down = SV[,names(SV) %in% c(down_df$Feature_ID)]
  di = vector( mode = 'numeric',length = dim(SV)[1] )
  for ( i in 1:dim(SV)[1])
  {
    di[i] = log2( ( sum(SV_down[i,]>0)+1 ) / (sum(SV_up[i,]>0)+1 )  )
  }
  return(di)
}

calculate_rank_health_index = function(SV, up_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-up_feature.txt', down_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-down_feature.txt')
{
  # rank
  pos = which( names(taxa) == 'Description' )
  sv_pos = (pos+1):dim(SV)[2]
  SV2 = SV[,sv_pos]
  for ( i in 1:dim(SV)[1] )
  {
    SV2[i,] = rank( SV2[i,] ) 
...
## calculating haya's microbial health index, the binary (and more common) version
calculate_binary_health_index_down = function(SV, up_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-up_feature.txt', down_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-down_feature.txt')
{
  up_df = read.table(up_file,sep="\t", header=TRUE, na.strings = na_str)
  down_df = read.table(down_file,sep="\t", header=TRUE, na.strings = na_str)
  SV_up = SV[,names(SV) %in% c(up_df$Feature_ID)]
  ...
  for ( i in 1:dim(SV)[1])
  {
    # di[i] = log2(  (sum(SV_up[i,]>0)+1 )  )
    di[i] = (  (sum(SV_up[i,]>0)+1 )  )
  }
  return(di)
}
...
## calculating haya's microbial health index, the binary (and more common) version
calculate_binary_health_index_up = function(SV, up_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-up_feature.txt', down_file = '/pita/users/tzipi/projects/16S/merges/lungs/DB_2_3_5_7_10_11_14_lungs_ctrl/R/haya_dysbiosis/data/nonspecific-down_feature.txt')
{
  up_df = read.table(up_file,sep="\t", header=TRUE, na.strings = na_str)
  down_df = read.table(down_file,sep="\t", header=TRUE, na.strings = na_str)
  SV_up = SV[,names(SV) %in% c(up_df$Feature_ID)]
  ...
  for ( i in 1:dim(SV)[1])
  {
    # di[i] = log2( sum(SV_down[i,]>0)+1 )
    di[i] = ( sum(SV_down[i,]>0)+1 )
  }
  return(di)
}

# checks if a string is only "normal" charterers or oncludes hebrew for example
# can take a c()
check_normal_text = function(text)
{
  return(!grepl("characters.unlist", iconv(text, "latin1", "ASCII", sub="characters.unlist")))
}

## add (n=) to a categorical vector
# for example c('Male','Male','Female') will become 
# c('Male (n=2)','Male (n=2)','Female (n=1)')
get_var_with_n = function(in_var)
{
  temp = in_var
  for ( i in 1:length( in_var ) )
  {
    in_var[i] = sprintf('%s (n=%d)', temp[i], sum( temp == temp[i], na.rm = T ) )
    if (is.na(temp[i]))
      in_var[i] = sprintf('%s (n=%d)', temp[i], sum( is.na(temp) ) )
  }
  return(in_var)
}


## filter non informative vars
#  every category with less than min_group_size samples is changed to NA
# afterwards  uninformative columns less than min_number_of_unique_vals categories are removed.
# keep if there are more than max_number_of_unique_vals_to_check - to avoid
# removing dates, IDs etc.
filter_metadata_informative = function(map, min_number_of_unique_vals = 2, 
                                       max_number_of_unique_vals_to_check = 20, 
                                       min_group_size = 5)
{
  bad_cols = c()
  for ( i in 1:dim(map)[2] )
  {
    # not many options, usually sample name, date numeric etc.
    if ( length(table(map[,i])) < max_number_of_unique_vals_to_check & 
         ( !is.numeric( map[,i] ) | all(map[,i] %in% c(0,1,NA)) ) ) 
    {
      temp = table(map[,i])
      low_freq_to_remove = names(temp)[temp < min_group_size]
      if ( length(low_freq_to_remove) != 0 )
      {
        print(sprintf('rm %s: %s', names(map)[i], paste(low_freq_to_remove, 
                                                        collapse = ' ')))
        map[,i][ map[,i] %in% low_freq_to_remove ] = NA
      }
      if ( length(table(map[,i] )) < min_number_of_unique_vals )
      {
        bad_cols = c(bad_cols, names(map)[i])
        # print(names(map)[i])
      }
    } else if ( length( table(map[,i]) ) == 1 )
    {
      bad_cols = c(bad_cols, names(map)[i])
      # print(names(map)[i])
    }
  }
  good_map = map[, !names(map) %in% bad_cols]
  bad_map = map[, names(map) %in% bad_cols]
  return(list( good_map, bad_map))
}



# normalize only the ffq variables in big taxa file to "ffq_norm_var"
# ffq variables are from "first_ffq" to "last_ffq"
# also remove variables with "no_norm_var_part" as part of their name
norm_ffq_by_energy = function(taxa, first_ffq = 'Water_g.d', 
                              last_ffq = 'Fiber_g.1000kcal',
                              ffq_norm_var = 'Energy_kcal.d',
                              no_norm_var_part = 'kcal', 
                              no_norm_vars = c('I.MEDAS_SCORE'))
{
  ffq_pos = which(names(taxa) == first_ffq):which(names(taxa) == last_ffq)
  ffq_pos = ffq_pos[!names(taxa)[ffq_pos] %in% no_norm_vars]
  already_norm_vars = c()
  for ( i in ffq_pos )
  {
    # if (names(taxa)[i] != ffq_norm_var)
    if ( !grepl(no_norm_var_part,names(taxa)[i] ) )
    {
      # if ( !names(taxa)[i] %in% no_norm_vars ) # ffq we do not want to normalize, such as I.MEDAS
      taxa[i] = taxa[i] / taxa[[ffq_norm_var]]
    } else
    {
      if ( names(taxa)[i] != ffq_norm_var )
      {
        ## marking already normalized variables to avoid repeats 
        # for example I do not need both Protein_g.d and Protein_per_kcal
        # after normalizing everything to Energy_kcal.d
        already_norm_vars = c(already_norm_vars, names(taxa)[i])
      }
    }
  }
  taxa = taxa[, !names(taxa) %in% already_norm_vars]
  return(taxa)
}
...
# ffq variables are from "first_ffq" to "last_ffq"
# also remove variables with "no_norm_var_part" as part of their name
# v2 = norm to 1000kcal instead of 1kcal
# also add suffix to col names of normalized values
norm_ffq_by_energy_v2 = function(taxa, first_ffq = 'Water_g.d', 
                              last_ffq = 'Fiber_g.1000kcal',
                              ffq_norm_var = 'Energy_kcal.d',
                              no_norm_var_part = 'kcal', 
                              no_norm_vars = c('I.MEDAS_SCORE'),
                              norm_suffix = '_cal',
                              rm_vars = c('Carbohydrates_g.d','Protein_g.d',
                                          'Added_sugar_g.d','Tot_fat_g.d',
                                          'SFA_g.d','MUFA_g.d','PUFA_g.d',
                                          'Calcium_mg.1000kcal','Fiber_g.1000kcal'))
{
  normed_vars = c()
  ffq_pos = which(names(taxa) == first_ffq):which(names(taxa) == last_ffq)
  ffq_pos = ffq_pos[!names(taxa)[ffq_pos] %in% no_norm_vars]
  ...
  for ( i in ffq_pos )
  {
    ...
    {
      # if ( !names(taxa)[i] %in% no_norm_vars ) # ffq we do not want to normalize, such as I.MEDAS
      taxa[i] = ( taxa[i] / taxa[[ffq_norm_var]] ) * 1000
      normed_vars = c(normed_vars, names(taxa)[i])
    } else
    {
      if ( names(taxa)[i] != ffq_norm_var )
      # if ( grepl('_g.1000kcal',names(taxa)[i] )  )
      {
        ## marking already normalized variables to avoid repeats 
        # for example I do not need both Protein_g.d and Protein_per_kcal
        # after normalizing everything to Energy_kcal.d
  ...
  # taxa = taxa[, !names(taxa) %in% already_norm_vars]
  taxa = taxa[, !names(taxa) %in% rm_vars]
  ## add norm suffix
  pos = names(taxa) %in% normed_vars
  names(taxa)[pos] = sprintf('%s%s', names(taxa)[pos], norm_suffix)
  return(list(taxa, normed_vars))
}

show_rgb = function(cols)
{
  plotly::showRGB(plotly::toRGB(cols))
}

rbind_with_fill = function(x, y)
{
  m1 = names(x)[!names(x) %in% names(y)]
  for (v in m1) { y[[v]] = NA }
  m2 = names(y)[!names(y) %in% names(x)]
  for (v in m2) { x[[v]] = NA }
  return(rbind(x,y))
} 

# organizes dates list, changing "." and "-" to "_"
# remove time in day if in format DATE:TIME
# and handle partial dates (=only year or tear and month)
# assumes d-m-Y or Y-m-d format
clean_dates = function(dates)
{
  dates = gsub('_[0-9]+[:].*','',dates)
  dates = gsub('[.]','_',dates)
  dates = gsub('[-]','_',dates)
  
  # handle partial dates (=only year or tear and month)
  count_ = stringr::str_count(dates, pattern = '_')
  pos = count_ == 1 & !is.na(count_)
  dates[pos] = sprintf('01_0%s',dates[pos]); 
  dates[pos] = gsub('_012_','_12_',dates[pos]); 
  dates[pos] = gsub('_011_','_11_',dates[pos]); 
  dates[pos] = gsub('_010_','_10_',dates[pos]); 
  pos = count_ == 0 & !is.na(count_)
  dates[pos] = sprintf('01_01_%s',dates[pos]); 
  
  # if date is reversed = Y-m-d
  pos = which(as.numeric(gsub('_.*','',dates)) > 1000)
  # switch to d_m_Y
  dates[pos] = format( as.Date(dates[pos] , format = '%Y_%m_%d'), '%d_%m_%Y')
